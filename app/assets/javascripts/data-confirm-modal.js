...
(function ($) {
  ...
  var buildModal = function (options) {
    ...
    var modalClose = '<button type="button" class="close modal-close" data-dismiss="modal" aria-hidden="true"><span>×</span></button>'
      ...
      '<div id="'+id+'" class="modal '+modalClass+' '+fade+'" tabindex="-1" role="dialog" aria-labelledby="'+id+'Label" aria-hidden="true">' +
        '<div class="modal-dialog modal-dialog-centered" role="document">' +
          '<div class="modal-content">' +
            modalClose +
            '<div class="modal-body modal-data-confirm"></div>' +
            '<div class="modal-footer">' +
              '<button class="btn commit btn-confirm-delete"></button>' +
              '<button class="btn cancel btn-cancel-delete" data-dismiss="modal" aria-hidden="true"></button>' +
            '</div>'+
          '</div>'+
        '</div>'+
      '</div>'
    ...
    $.each((options.text||'').split(/\n{2}/), function (i, piece) {
      body.append($('<h2/>').html(piece).addClass('modal-title text-center'));
    });
