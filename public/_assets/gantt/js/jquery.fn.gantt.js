...
(function ($, undefined) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings = {
            ...
            minScale: "hours",
            // custom settings
            daysScaleCellWidth: undefined,
            weeksScaleCellWidth: undefined,
            monthsScaleCellWidth: undefined,
        ...
        var core = {
            ...
            init: function (element) {
                ...
                // Iniitializing Scale select
                var scaleSelect = $('.scale select:not(.initialized)');
                var scalesRelationArr = ['days', 'weeks', 'months'];
                if (scaleSelect.length) {
                    scaleSelect.addClass('initialized');
                    scaleSelect.val(settings.scale);
                    scaleSelect.on('change', function(e) {
                        core.zoomInOut(element, $(e.target).val());
                    });
                }
            },
            ...
            render: function (element) {
                ...
                var viewport = document.querySelector('.gantt .rightPanel');
                var scrollable = document.querySelector('.gantt .dataPanel');

                if (core._tempScrollBoosterObj && typeof core._tempScrollBoosterObj.destroy === 'function') {
                    core._tempScrollBoosterObj.destroy();
                }

                core._tempScrollBoosterObj = new ScrollBooster({
                  viewport: viewport,
                  content: scrollable,
                  mode: 'x',
                ...
                window._tempScrollBoosterObj = core._tempScrollBoosterObj;
                ...
                $dataPanel.css({
                    height: (Number($dataPanel.find('.bar:last').css('top').replace('px', '')) + 30) + 'px'
                    // height: (tools.getCellSize() * element.headerRows + $('div[id*=rowheader]').length) + 30) + 'px'
                });
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage &&
                        ...
                        entries.push(
                            ...
                            '" id="rowheader' + i +
                            '" data-offset="' + i % settings.itemsPerPage * (tools.getCellSize() + 10) + '">' +
                            '<span class="fn-label' +
            ...
            rightPanel: function (element) {
                ...
                case "weeks":
                    ...
                    calcWidth = tools.getCellWidth(range.length, settings.weeksScaleCellWidth);
                    dataPanelWidth = range.length * calcWidth;
                    ...
                    for (i = 0, len = range.length; i < len; i++) {
                        ...
                        if (week > (week = rday.getWeekOfYear())) {
                            ...
                            yearArr.push(
                                '<div class="row year" style="width: ' +
                                calcWidth * (scaleUnitsThisYear - diff) +
                                'px;"><div class="fn-label">' +
                                year +
                                '</div></div>');
                            year++;
                        ...
                        if (rday.getMonth() !== month) {
                            ...
                            monthArr.push(
                                '<div class="row month" style="width:' +
                                calcWidth * (scaleUnitsThisMonth - diff) +
                                'px;"><div class="fn-label">' +
                                settings.months[month] +
                                '</div></div>');
                            month = rday.getMonth();
                        ...
                        // Fill weeks
                        dayArr.push(
                            '<div style="width:' + calcWidth + 'px;" class="row day wd"' +
                            ' id="' + rday.getWeekId() +
                            '" data-offset="' + i * calcWidth +
                            '" data-repdate="' + rday.getRepDate(settings.scale) + '">' +
                    ...
                    yearArr.push(
                        '<div class="row year" style="width: ' +
                        calcWidth * scaleUnitsThisYear + 'px;"><div class="fn-label">' +
                        year +
                        '</div></div>');
                    ...
                    monthArr.push(
                        '<div class="row month" style="width: ' +
                        calcWidth * scaleUnitsThisMonth + 'px"><div class="fn-label">' +
                        settings.months[month] +
                        '</div></div>');
                ...
                case 'months':
                    ...
                    calcWidth = tools.getCellWidth(range.length, settings.monthsScaleCellWidth);
                    dataPanelWidth = range.length * calcWidth;
                    ...
                    for (i = 0, len = range.length; i < len; i++) {
                        ...
                        scaleUnitsThisYear++;
                        monthArr.push(
                            '<div style="width:' + calcWidth + 'px;" class="row day wd" id="dh-' + tools.genId(rday) +
                            '" data-offset="' + i * calcWidth +
                            '" data-repdate="' + rday.getRepDate(settings.scale) + '">' +
                            settings.months[rday.getMonth()] + '</div>');
                    }
                ...
                default:
                    ...
                    calcWidth = tools.getCellWidth(range.length, settings.daysScaleCellWidth);
                    dataPanelWidth = range.length * calcWidth;
                    ...
                    for (i = 0, len = range.length; i < len; i++) {
                        ...
                        dayArr.push(
                            '<div style="width:' + calcWidth + 'px;" class="row date ' + dayClass + '"' +
                            ' id="dh-' + tools.genId(rday) +
                            '" data-offset="' + i * calcWidth +
                            '" data-repdate="' + rday.getRepDate(settings.scale) + '">' +
                            '<div class="fn-label">' + rday.getDate() + '</div></div>');
                        dowArr.push(
                            '<div style="width:' + calcWidth + 'px;" class="row day ' + dayClass + '"' +
                ...
                return $('<div class="rightPanel"></div>')
                    .addClass('custom_' + settings.scale)
                    .append(dataPanel);
            },
            ...
            createProgressBar: function (entry, subentry) {

                var label = subentry.isLower ? '' : $('<div class="fn-label">' + (entry.name || '') + '</div>');
                var arrows = subentry.isLower ? '' : $('<div class="arrow-right"></div><div class="arrow-left"></div>');
                var popupHTML = '';

                var plannedTiming = entry.values.map(function(element) {
                    if (element.label === 'Planned time UTC') {
                ...
                var factTiming = entry.values.map(function(element) {
                    if (element.label === 'Fact time UTC') {
                        return element.from + ' — ' + element.to;
                    } else {
                        return '';
                    }
                ...
                //if (entry.desc) {
                    ...
                    popupHTML =
                        ...
                        '<div class="hint_info_block">' +
                            '<div><p>Budget:</p><p>' + entry.dataObj.budget + '</p></div>' +
                            '<div><p>Owner:</p><p>' + entry.dataObj.owner + '</p></div>' +
                            '<div><p>Contractor:</p><p>' + entry.dataObj.contractor + '</p></div>' +
                        '</div>' +
                        '<div class="hint_time">' +
                            '<div><div>Planned time (UTC):</div><div class="planned_legend"></div></div>' +
                            '<p>' + plannedTiming + '</p>' +
                        '</div>' +
                        '<div class="hint_time">' +
                            '<div><div>Fact time (UTC):</div><div class="fact_legend"></div></div>' +
                            '<p>' + factTiming + '</p>' +
                        '</div>';
            ...
            fillData: function (element, datapanel) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage &&
                        ...
                        $.each(entry.values, function (j, subentry) {

                            var _bar;
                            var from, to, cFrom, cTo, dFrom, dTo, dl, dp;
                            var topEl, top;
                            var customHeightOffset = subentry.isLower ? settings.taskBarHeight : 0;

                            switch (settings.scale) {
                            // **Hourly data**
                            case "hours":
                                dFrom = tools.genId(tools.dateDeserialize(subentry.from), element.scaleStep);
                                from = $(element).find('#dh-' + dFrom);
                                dTo = tools.genId(tools.dateDeserialize(subentry.to), element.scaleStep);
                                ...
                                _bar = core.createProgressBar(entry, subentry);

                                // find row
                                topEl = $(element).find("#rowheader" + i);
                                top = cellWidth * 5 + barOffset + topEl.data("offset") + customHeightOffset;
                                _bar.css({
                                  top: top,
                            ...
                            // **Weekly data**
                            case "weeks":
                                dFrom = tools.dateDeserialize(subentry.from);
                                dTo = tools.dateDeserialize(subentry.to);

                                from = $(element).find("#" + dFrom.getWeekId());
                                cFrom = from.data("offset");
                                to = $(element).find("#" + dTo.getWeekId());
                                cTo = to.data("offset");
                                dl = Math.round((cTo - cFrom) / tools._cellWidth) + 1;
                                dp = 100 * (tools._cellWidth * dl - 1) / dataPanelWidth;

                                _bar = core.createProgressBar(entry, subentry);

                                // find row
                                topEl = $(element).find("#rowheader" + i);
                                top = cellWidth * 3 + barOffset + topEl.data("offset") * 2 + customHeightOffset + 30;
                                _bar.css({
                                  top: top,
                                  left: Math.floor(cFrom),
                                  width: dp + '%'
                            ...
                            case "months":
                                ...
                                // find row
                                topEl = $(element).find("#rowheader" + i);
                                top = cellWidth * 2 + barOffset + topEl.data("offset") * 2 + customHeightOffset + 30;
                                _bar.css({
                                  top: top,
                                  left: Math.floor(cFrom),
                                  width: dp + '%'
                            ...
                            case "days":
                                /* falls through */
                            default:
                                dFrom = tools.genId(tools.dateDeserialize(subentry.from));
                                dTo = tools.genId(tools.dateDeserialize(subentry.to));
                                from = $(element).find("#dh-" + dFrom);
                                cFrom = from.data("offset");
                                ...
                                // find row
                                topEl = $(element).find("#rowheader" + i);
                                top = cellWidth * 4 + barOffset + topEl.data("offset") * 2 + customHeightOffset + 15;
                                _bar.css({
                                  top: top,
                                  left: Math.floor(cFrom),
                                  width: dp + '%'
            ...
            zoomInOut: function (element, val) {
                core.waitToggle(element, function () {
                    ...
                    element.scaleStep = newStepData.scaleSt;
                    settings.scale = val;
                    element.headerRows = newStepData.headerRows;
                    var $rightPanel = $(element).find(".fn-gantt .rightPanel");
                    var $dataPanel = $rightPanel.find(".dataPanel");
        ...
        var tools = {
            ...
            getCellWidth: function(numberOfCells, userDefinedValue) {

                var containerWidth = Number(jQuery('.gantt').width());
                var cellWidth = userDefinedValue || (containerWidth / numberOfCells);
                tools._cellWidth = ((cellWidth < 28) ? 28 : cellWidth);

                return tools._cellWidth;
