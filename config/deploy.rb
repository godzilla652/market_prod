...
set :repo_url, "git@github.com:Peluhnya/astra.git"

append :linked_dirs, 'log', 'tmp/pids', 'tmp/cache', 'tmp/sockets', 'vendor/bundle', 'public/system', 'public/uploads', 'public/assets'
append :linked_files, 'config/database.yml', 'config/sidekiq.yml', 'config/environments/production.rb', 'config/application.yml'

set :rvm_map_bins, %w{gem rake ruby rails bundle sidekiq sidekiqctl puma pumactl}

set :preload_app, true
set :rvm_custom_path, '/home/astra/.rvm'
  ...
  task :check_revision do
    on roles(:app) do
      ...
      unless `git rev-parse HEAD` == `git rev-parse origin/#{fetch(:branch)}`
        puts "WARNING: HEAD is not the same as origin/roles"
